...
/**
 * Here we are going to use QueryDSL rather than JPQL or the Criteria API
 */
@Transactional
public class JPAQueryDslDAO implements BaseDAO<Track> {
    ...
    public List<Track> getByFlexiSearch(SearchWrapper searchWrapper) {
        ...
        if (searchWrapper.operation == SearchSpec.Operation.Or) {
            finalPredicate = builder.or(preds.toArray(new Predicate[0]));
        } else {
            finalPredicate = builder.and(preds.toArray(new Predicate[0]));
        }
